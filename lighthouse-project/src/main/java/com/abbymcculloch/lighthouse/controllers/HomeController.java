...
//	put this check in all gets
	...
	messageList.removeIf(message -> message.isMessageAvailable());
...
public String profile(HttpSession session, Model model, @PathVariable("id") Long id) {
//	put this check in all gets
	if(session.getAttribute("userId") == null) {
		return "redirect:/";
	}
...
@PostMapping("/profile/{id}/setpublic")
public String privacySettingsPublic(
		@PathVariable("id") Long id, Model model) {
	User userSetting = userService.findUser(id);
	Boolean setting = userSetting.getPrivacy();
	setting = false;
	userSetting.setPrivacy(setting);
	userSetting.setConfirm(userSetting.getPassword());
	userService.updateUser(userSetting);
	return "redirect:/profile/{id}";
}

@PostMapping("/profile/{id}/setprivate")
public String privacySettingsPrivate(
		@PathVariable("id") Long id, Model model) {
	User userSetting = userService.findUser(id);
	Boolean setting = userSetting.getPrivacy();
	setting = true;
